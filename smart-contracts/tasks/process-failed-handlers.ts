...
task(
  ...
  console.log(`Processing ${TX_IDS.length} transactions from Zero network...\n`)
  ...
  for (let i = 0; i < TX_IDS.length; i++) {
    ...
    try {
      ...
      for (const log of receipt.logs) {
        if (
          log.address.toLowerCase() !== HYPERLANE_MAILBOX_ADDRESS.toLowerCase()
        ) {
          continue
        }
  ...
  console.log('\n=== Summary ===')
  console.log(`Total transactions processed: ${calls.length}/${TX_IDS.length}`)
  console.log(
